...
/**
 * This class provides complex Commands that are used by both RobotContainer and
 * NamedCommands
 */
public class SuperStructure {
    // subsystems
    private AutoAimer aimer;
    private ShotInfo shotInfo = new ShotInfo(
    ...
    private final LoggedNetworkNumber turretTolDeg = new LoggedNetworkNumber("Turret/turretTolDeg", 4);
    ...
    /**
     * The minimum difference to block the indexer from running, in rotations per
     * second
     */
    private final LoggedNetworkNumber shooterTolRPS = new LoggedNetworkNumber("Shooter/shooterTolRPS", 4.0);
    /**
     * initial tolerance before indexer starts running at all (should be more strict
     * than general
     * tolerance)
     */
    private final LoggedNetworkNumber shooterInitialTolRPS = new LoggedNetworkNumber("Shooter/shooterTolRPS", 4.0);
    /** The duty cycle speed to use while intaking [-1.0, 1.0] */
    private final LoggedNetworkNumber intakeSpeed = new LoggedNetworkNumber("Intake/intakeSpeed", 70.0);
    ...
    /** Offset for hood. Applied while shooting */
    private final LoggedNetworkNumber turretOffset = new LoggedNetworkNumber("Offsets/turretOffset", 0.0);
    /** Offset for hood. Applied while shooting */
    private final LoggedNetworkNumber shooterOffset = new LoggedNetworkNumber("Offsets/shooterOffset", 0.0);

    private final LoggedNetworkNumber driveWiggle = new LoggedNetworkNumber("Offsets/driveWiggle", 0.05);
    private final LoggedNetworkNumber driveWigglePeriod = new LoggedNetworkNumber("Offsets/driveWigglePeriod", 1);

    private final LoggedNetworkNumber driveShake = new LoggedNetworkNumber("Offsets/driveShake", 0.1);
    private final LoggedNetworkNumber driveShakePeriod = new LoggedNetworkNumber("Offsets/driveShakePeriod", 1);
    private final LoggedNetworkNumber intakeFlipPeriod = new LoggedNetworkNumber("Offsets/intakeFlipPeriod", 1);

    private final LoggedNetworkNumber intakeUpSetpoint = new LoggedNetworkNumber("Intake/flipUpSetpoint", 88.2);
    private final LoggedNetworkNumber intakeDownSetpoint = new LoggedNetworkNumber("Intake/flipDownSetpoint", 18);
    ...
    public Command shoot() {
        return Commands.run(
                () -> {
                    ...
                    offsets.vxMetersPerSecond = (drive.getRotation().getSin() * driveWiggle.get())
                            * Math.sin(
                                    Timer.getFPGATimestamp() * (2 * Math.PI) / driveWigglePeriod.get());
                    offsets.vyMetersPerSecond = (drive.getRotation().getCos() * driveWiggle.get())
                            * Math.sin(
                                    Timer.getFPGATimestamp() * (2 * Math.PI) / driveWigglePeriod.get());
                    offsets.omegaRadiansPerSecond = driveShake.get()
    ...
    public Command justShoot() {
        return shoot().alongWith(flips.runFlips(
                () -> Degrees.of(
                        ...
                        .andThen(
                                indexer
                                        .runBoth(
                                                chuteSpeed,
                                                spinSpeed)
                                        .alongWith(intake.runIntake(() -> RotationsPerSecond.of((intakeSpeed.get()))))
                                        .onlyWhile(() -> shooter.meetsSetpoint(shooterTolRPS).getAsBoolean()
                                                && turret.isAtSetpoint(Degrees.of(turretTolDeg.get())))
                                        .onlyIf(() -> shooter.meetsSetpoint(shooterTolRPS).getAsBoolean()
                                                && turret.isAtSetpoint(Degrees.of(turretTolDeg.get())))
    ...
    public Command shootAndIntake() {
        return shoot().alongWith(flips.runFlips(
                () -> Degrees.of(intakeDownSetpoint.get())),
                Commands.waitUntil(shooter.meetsSetpoint(shooterInitialTolRPS))
                        .andThen(
                                indexer
                                        .runBoth(
                                        ...
                                        .repeatedly()),
                intake.runIntake(() -> RotationsPerSecond.of(intakeSpeed.get()))).withInterruptBehavior(InterruptionBehavior.kCancelSelf);
    }
    ...
    public Command runIntake() {
        ...
        return flipIntakeDown()
                .andThen(intake.runIntake(() -> RotationsPerSecond.of(intakeSpeed.get()))).withInterruptBehavior(InterruptionBehavior.kCancelSelf);
    ...
    public BooleanSupplier isHoodAngleRight() {
        return () -> Math
                .abs(hood.getHood() - (shotInfo.shooterParameters().hood() + hoodOffset.getAsDouble())) < hoodTol.get();
    }
    ...
    /**
     * Computes the position to aim at. If robot is in the alliance zone, the hub's
     * pose will be
     * returned. If the robot is in the neutral or enemy alliance zone, a pose at
     * (2, y) will be
     * returned. The pose will be adjusted the line between the robot and it
     * intersects the hub.
     *
